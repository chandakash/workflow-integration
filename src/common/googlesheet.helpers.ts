...
import { IDataObject, SheetDataRow, SheetRangeData } from './interfaces';
import { isEqualLoadash } from './lodash';
...
export async function getRevisionFile(access_token: string, exportLink: string) {
...
export function compareRevisions(
	...
	for (let i = dataStartIndex; i < dataLength; i++) {
		...
		if (columnsToWatch?.length) {
			...
			if (isEqualLoadash(currentRow, previousRow)) continue;
		} else if (isEqualLoadash(current[i], previous[i])) continue;
	...
	if (includeInOutput === 'both') {
		...
		const differences = currentData.map(({ current: currentRow }, index) => {
			...
			Object.keys(rest).forEach((key) => {
				...
				if (isEqualLoadash(previousValue, currentValue)) return;
